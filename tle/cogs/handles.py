...
def get_gudgitters_image(
    ...
    for i, (pos, name, handle, rating, score) in enumerate(rankings):
        ...
        draw_row(
            str(pos + 1),
            name,
            f'{handle} ({rating if rating else "N/A"})',
            str(score),
            color,
            y,
        )
...
def get_prettyhandles_image(
    rows: list[tuple[int, str, str, int | None]], font: Any
) -> Image.Image:
    """return PIL image for rankings"""
    SMOKE_WHITE = (250, 250, 250)
    BLACK = (0, 0, 0)
...
class Handles(commands.Cog):
    def __init__(self, bot: commands.Bot) -> None:
        ...
        self.font = ImageFont.truetype(
            str(constants.NOTO_SANS_CJK_BOLD_FONT_PATH), size=26
        )
    ...
    async def monthlygudgitters(self, ctx, *, args: str = '') -> None:
        ...
        results = await self.bot.user_db.get_gudgitters_timerange(start_time, end_time)
        res = {}
        ...
        for user_id, score in sorted(res.items(), key=lambda item: item[1], reverse=True):
            ...
            if score > 0:
                ...
                rating_changes = await cache.get_rating_changes_for_handle(handle)
    ...
    async def pretty(
        self, ctx: commands.Context, page_no: int | None = None
    ) -> None:
        """Show members of the server who have registered their handles and
        their Codeforces ratings, in color.
        """

...
| **Duel** | 1v1 problem duels between server members |
| **Training** | assigns a ramping series of problems to solve |
| **Lockout** | team-vs-team lockout rounds |
...
| `TLE_ADMIN` | ❌ | `Admin` | role name (or id) that can run admin cmds |
| `TLE_MODERATOR` | ❌ | `Moderator` | role name (or id) that can run mod cmds |
| `TLE_TRUSTED` | ❌ | `Trusted` | role name (or id) that can run trusted cmds |
| `TLE_PURGATORY` | ❌ | `Purgatory` | role excluded from the "orz" presence rotation |
